...
namespace EquitWebServer {
  ...
  *object.
  **
  ** @note If you create subclasses you MUST call this constructor in your
  *derived
  ** class constructors otherwise the socket may not be properly initialised to
  ** work in your handler.
  ** @note If you create subclasses of bpWebServer you MUST ensure that the
  *spawned
  ** handler threads receive sockets in the appropriate state.
  */
		RequestHandler(QTcpSocket * socket, const Configuration & opts,
							QObject * parent);

		/**
  ** @brief Destructor.
  **
  ** The desctructor does little of note beyond ensuring any
  ...
  ** @note This method assumes that the member m_socket is initialised and is
  *both
  ** readable and writeable.
  */
		void handleHTTPRequest(const std::string & version, const std::string & method, const std::string & uri, const HttpHeaders & headers,
  ...
  */
		bool sendError(int n, const QString & msg = QString::null,
  ...
  */
		...
		/*
  ** needs to be dynamically allocated because static allocation would occur at
  *ccontruction
  ** of the handler object, which means the socket would be owned by the server
  *thread, not
  ** the handler thread, and sockets cannot be controlled between threads. in
  *other words,
  ** the handler thread must own the socket object.
  */
		QTcpSocket * m_socket;  ///< The TCP socket for the request being handled.
		Configuration
		  m_config;					///< The configuration of the server responding to the request.
