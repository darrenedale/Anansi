...
EquitWebServer::MainWindow::~MainWindow( void ) {
	...
	if(m_controller) m_controller->setServer(0);
	m_server = 0;
	saveRecentConfigs();
}
...
void EquitWebServer::MainWindow::slotDocumentRootChanged( void ) {
	if(m_server->isListening()) QMessageBox::warning(this, tr("Set document root"), tr("The document root was changed and the server is currently running. This means that the actual document root being used to serve content will not be altered until the server is restarted. Content will continue to be served from the document root that was set when the server was last started."));
}
...
void EquitWebServer::MainWindow::loadConfiguration( const QString & fileName ) {
	...
	if(newConfig.load(fileName)) {
		...
		if(!m_recentConfigs.contains(fileName)) {
			...
			foreach(QAction * a, m_recentConfigsMenu->actions()) {
				if(a->data().toString() == fileName) a->setChecked(true);
			}
		}
...
bool EquitWebServer::MainWindow::startServer( void ) {
	...
	if(m_server->listen()) {
		statusBar()->showMessage(tr("The server is listening on %1:%2.").arg(m_server->configuration().getListenAddress()).arg(m_server->configuration().port()));
...
void EquitWebServer::MainWindow::about( void ) {
	QMessageBox::about(this, tr("About %1").arg(qApp->applicationName()),
					   ...
					   "<p style=\"font-weight: normal;\"><small>" + qApp->applicationName() + " uses the Qt toolkit (<a href=\"http://qt.nokia.com/\">http://qt.nokia.com/</a>).</small></p>"

					   "<p style=\"font-weight: normal;\"><small>" + qApp->applicationName() + " uses some icons from the KDE <a href=\"http://www.oxygen-icons.org/\">Oxygen</a> icon project, licensed under the <a href=\"http://www.gnu.org/licenses/lgpl-3.0.txt\">LGPLv3</a>.</small></p>");
}
